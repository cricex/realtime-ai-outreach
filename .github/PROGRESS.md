...
- **2026-04-10 21:20 UTC** — Enabled caller audio transcription (AudioInputTranscriptionOptions model=whisper-1). Both sides of conversation now captured in call history.
- **2026-04-10 20:45 UTC** — v2 fully deployed to Azure: Frontend build successful (216 KB JS, 19 KB CSS gzip), Docker image built, pushed to ACR (digest sha256:8d56b942...), Container App updated + health endpoint verified.
- **2026-04-10 20:15 UTC** — UI fixes: scenario title as prominent heading at top of editor, updates on AI generation. Scenario dropdown fetches fresh from API on selection. Multiple React state flow fixes.
- **2026-04-10 19:55 UTC** — Meta-prompt now read dynamically from disk (no restart needed). Re-wired prompts/meta_generate.md as system prompt for AI generation.
- **2026-04-10 19:30 UTC** — Auth gate: empty DEMO_PASSWORDS= disables auth for local dev. Added GET /auth/status endpoint for frontend to check if auth is required.
- **2026-04-10 19:15 UTC** — AI generation fixed: replaced Azure OpenAI with proper Microsoft Foundry (New) resource (allowProjectManagement=true). Deployed gpt-4.1-mini. Switched from azure-ai-inference to openai SDK (AsyncAzureOpenAI). Simplified meta-prompt to avoid content filter jailbreak triggers.
- **2026-04-10 17:30 UTC** — UI overhaul: 60/40 layout, inline phone dialer with keyboard support, RMS-based waveform display, scenario title in header dropdown.
- **2026-04-10 14:35 UTC** — v2 development branch established with Phase 1 refactor complete. 87 files changed, +8224 / -2280 lines.
